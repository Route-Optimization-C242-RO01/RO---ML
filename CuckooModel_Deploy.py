...
if response.status_code == 200:
    data_input = response.json()  # Parsing JSON dari respons
else:
    print(f"Request gagal dengan status code: {response.status_code}")

def Convert_Json (data_json) : 
     # Contoh: Mengakses elemen tertentu dari data
    if "Street" in data_json:
        Nama_Jalan = data_json["Street"]
    if "City" in data_json:
        Kota = data_json["City"]
    if "Province" in data_json:
        Provinsi = data_json["Province"]
    if "Kg" in data_json:
        Kapasitas = data_json["Kg"]
    if "Number of Vehicles" in data_json :
        Banyak_Kendaraan = data_json["Number of Vehicles"]
    Addresses = [f"{a},{b},{c}" for a, b, c in zip(Nama_Jalan, Kota, Provinsi)]
    return Addresses,Kapasitas , Banyak_Kendaraan
...
addresses,Kapasitas,Banyak_Kendaraan = Convert_Json (data_input)
...
def get_detailed_solution () :
    ...
    for sol in final_routes :
        ...
        for pelanggan in sol :
            Coordinate.append((Latitude[pelanggan],Longitude[pelanggan]))
        Koordinat["Kendaraan" + str(Kendaraan)] =Coordinate

    solution = OrderedDict({
        "Koordinat": (Koordinat),
        "Total_Jarak Ditempuh ": (best_distance)
    })
    return solution

s = get_detailed_solution()
print(s)

class OptimizeRoute(Resource):
    def get(self):
        solution  = get_detailed_solution()
        return jsonify(solution)
api.add_resource(OptimizeRoute, '/api/Solution')
